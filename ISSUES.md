...
**Status:** ✅ fixed — root cause: FB 2026 đổi profile link sang `/people/<Tên>/pfbid…/`, UID dạng pfbid (chữ-số) không khớp regex `\d+`. Đã fix clean_facebook_url + extract_uid_from_url (app.py) + uid_from_url (exporter.py), thêm test [1b] pfbid, test_exporter.py PASS. Verify: 1 bài group → 3 lead có SĐT (trước 0).
